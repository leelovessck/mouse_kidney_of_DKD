...
####载入数据###############
#载入数据
ALL.DEG <- read.csv("./result/5.2 全部细胞DEG/全部MC的DEG.csv")
...
kegg <- setReadable(kegg,"org.Mm.eg.db","ENTREZID")  
  ##把ENTREZID转换为SYMBOL
write.csv(kegg@result, "1.1 全部MC的KEGG（all）.csv")
...
goMF <- enrichGO(ALL.DEG_df$SYMBOL, 
                 ...
                 readable = FALSE, 
                 pool = FALSE)
write.csv(goMF@result, "1.2 全部MC的GO_MF（all）.csv")
...
goCC <- enrichGO(ALL.DEG_df$SYMBOL, 
                 ...
                 readable = FALSE, 
                 pool = FALSE)
write.csv(goCC@result, "1.3 全部MC的GO_CC（all）.csv")
...
goBP <- enrichGO(ALL.DEG_df$SYMBOL, 
                 ...
                 readable = FALSE, 
                 pool = FALSE)
write.csv(goBP@result, "1.4 全部MC的GO_BP（all）.csv")
...
Reactome <- setReadable(Reactome,"org.Mm.eg.db","ENTREZID")  
  ##把ENTREZID转换为SYMBOL
write.csv(Reactome@result, "1.5 全部MC的Reactome（all）.csv")
...
kegg <- setReadable(kegg,"org.Mm.eg.db","ENTREZID")  
  ##把ENTREZID转换为SYMBOL
write.csv(kegg@result, "2.1 全部MC的KEGG（up）.csv")
...
goMF <- enrichGO(UP.DEG_df$SYMBOL, 
                 ...
                 readable = FALSE, 
                 pool = FALSE)
write.csv(goMF@result, "2.2全部MC的GO_MF（up）.csv")
...
goCC <- enrichGO(UP.DEG_df$SYMBOL, 
                 ...
                 readable = FALSE, 
                 pool = FALSE)
write.csv(goCC@result, "2.3 全部MC的GO_CC（up）.csv")
...
goBP <- enrichGO(UP.DEG_df$SYMBOL, 
                 ...
                 readable = FALSE, 
                 pool = FALSE)
write.csv(goBP@result, "2.4 全部MC的GO_BP（up）.csv")
...
Reactome <- setReadable(Reactome,"org.Mm.eg.db","ENTREZID")  
##把ENTREZID转换为SYMBOL
write.csv(Reactome@result, "2.5 全部MC的Reactome（up）.csv")
...
kegg <- setReadable(kegg,"org.Mm.eg.db","ENTREZID")  
  ##把ENTREZID转换为SYMBOL
write.csv(kegg@result, "3.1 全部MC的KEGG（down）.csv")
...
goMF <- enrichGO(DOWN.DEG_df$SYMBOL, 
                 ...
                 readable = FALSE, 
                 pool = FALSE)
write.csv(goMF@result, "3.2 全部MC的GO_MF（down）.csv")
...
goCC <- enrichGO(DOWN.DEG_df$SYMBOL, 
                 ...
                 readable = FALSE, 
                 pool = FALSE)
write.csv(goCC@result, "3.3 全部MC的GO_CC（down）.csv")
...
goBP <- enrichGO(DOWN.DEG_df$SYMBOL, 
                 ...
                 readable = FALSE, 
                 pool = FALSE)
write.csv(goBP@result, "3.4 全部MC的GO_BP（down）.csv")
...
Reactome <- setReadable(Reactome,"org.Mm.eg.db","ENTREZID")  
##把ENTREZID转换为SYMBOL
write.csv(Reactome@result, "3.5 全部MC的Reactome（down）.csv")
